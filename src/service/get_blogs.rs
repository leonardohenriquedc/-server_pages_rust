...
const PATH_FILES_BLOGS: &str = "content/";
const LINK_DEBUG: &str = "http://localhost:7878/";
...
pub async fn get_links_of_all_blogs() -> String {
    ...
    for name in name_blogs {
        let link = format!("{}post/{}", LINK_DEBUG, name);
...
pub async fn get_blog_post(name: String) -> String {
    let path_article = format!("{}{}", PATH_FILES_BLOGS, name);
